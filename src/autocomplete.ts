...
export class Autocomplete<TItem extends Item> {
    ...
    constructor(options: Options<TItem>) {
        ...
        if (options.autoDiscover) {
            this.input ||= this.root.querySelector(
                `.${options.autoDiscover}-input`
            );
            this.panel ||= this.root.querySelector(
                `.${options.autoDiscover}-panel`
            );
            this.list ||= this.root.querySelector(
                `.${options.autoDiscover}-list`
            );
            this.label ||= this.root.querySelector(
                `.${options.autoDiscover}-label`
            );
            this.form ||= this.root.querySelector(
                `.${options.autoDiscover}-form`
            );
        }
    ...
    private createAutocomplete(options: Options<TItem>) {
        const autocomplete = createAutocomplete<TItem>({
            ...
            shouldPanelOpen: (props) => {
                const shouldPanelOpen = options.shouldPanelOpen?.call(
                    this,
                    props
                ) as boolean;
                ...
                const hasNoResultsTemplate = props.state.collections
                    .map(
                        (c) =>
                            c.source as AutocompleteSourceWithTemplates<TItem>
                    )
    ...
    private diffStates(
        ...
        const curr = state as unknown as Record<string, unknown>;
        const prev = prevState as unknown as Record<string, unknown>;

        const dOut = (value: unknown) =>
            String(value instanceof Array ? value.length : value);

        for (const key in curr) {
            if (curr[key] !== prev[key]) {
                this.log(
                    `Found diff! ${key}: ${dOut(prev[key])} -> ${dOut(
                        curr[key]
                    )}`
                );
                out.hasDiff = true;
